...
export default {
  ...
  methods: {
    getData() {
      ...
      newRequest({
        ...
        if (res.success == 1) {
          ...
          if (this.obj.beforeList.length && this.obj.beforeList[this.obj.beforeList.length - 1].productNumber == '合计') {
    ...
    checkOutInput() {
      ...
      console.log(this.requestParams.type, this.requestParams.lstClothLoneIdToReturnReplace.length, this.requestParams.reason, bool)
      return bool
    },
    // 处理确认"选好了"按钮事件
    confirmEditSubmit() {
    ...
    calQuantity() {
      if (!this.obj.beforeList.length) return;
