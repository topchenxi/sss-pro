      ...
      <div class="detail-item w100p p14">
        <template v-if="orderDetail">
          ...
          <el-dialog v-model="dialogVisible" size="small" custom-class="submit-pay-money" title="提交对账" :close-on-click-modal="false">
            ...
            <textarea rows="3" style="display: inline-block; width: 200px; resize: none;margin-left: 150px; padding:5px; height: 100px;" v-model.trim="remark"></textarea>
            <span>
    ...
    newRequest('/redwood/reconciliation/pay', {
        ...
        if (res.success == 1) {
          ...
          this.$message({
            ...
            duration: 1200,
            onClose() {
              self.$router.push({ name: 'guiderMoneWaitSubmit' })
            }
          })
        } else {
          this.$message({
