            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item,index) in list" :key="index">
                  <!-- 图片 -->
                  <td>
                    <a :name="index" :href="imageDomain + item.colorUrls[0]" v-lightbox="item.colorUrls" v-if="item.colorUrls.length > 0" v-bind:data="`商品` + index"> <img :src="imageDomain +  item.colorUrls[0]" width="60" height="60" /> </a> <img src="~assets/default_pic_detail.png" alt="" width="60px" height="60px" v-else>
                  </td>
                  <!-- 供应商 -->
                  <td>{{item.company || '--' }}</td>
                  <!-- 货号 -->
                  ...
                  <td>
                    ...
                    <el-button class="table-btn" @click.native="toEdit(item.id)">编辑</el-button>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  methods: {
    toDetail(id) {
      this.$router.push({ name: 'productDetail', query: { 'id': id, 'type': 0 } });
    },
    toEdit(id) {
