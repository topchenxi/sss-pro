<template>
  <section style="min-width:1300">
      ...
      <template>
        ...
        <div class="table-box">
          <el-table :data="item.list" class="fixed-table" border>
            ...
            <el-table-column label="财务处理时间" align="left" :formatter="formatTime" prop="financeReviewTime">
            </el-table-column>
            <el-table-column min-width="100px" fixed="right" label="操作">
              <template scope="scope">
                <router-link :to="{name: 'rrSellerRefundDetail', query:{id: scope.row.id, status: 7}}">
    ...
    <div class="page-box">
      <Pagination :pageSize="page.pageSize" :page="page.pageNumber" :total="page.totalCount" :options="requestParams" :render="getData"></Pagination>
    </div>
  </section>
</template>
