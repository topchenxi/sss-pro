...
<!-- 退换货信息 -->
    <div class="section" style="border-bottom: 0;">
      ...
      <div class="line-item clearfix">
        ...
        <div class="line-item-r">
          <el-radio-group v-model="changeType" size="small" @change="selectType">
            <el-radio-button :label="0">售前退货</el-radio-button>
            <el-radio-button :label="1">售前换货</el-radio-button>
            <!-- <el-radio :label="2">售后退货</el-radio>
      ...
      <div class="line-item clearfix">
        ...
        <div class="line-item-r" style="overflow-x: auto;">
           ...
           <el-table :data="colorList" border @selection-change="handleSelectionChange" @select="changeCurrent">
             <el-table-column type="selection"  width="55" :selectable="selectData"></el-table-column>
             <el-table-column prop="outReposityNumber" label="出仓单号"  v-if="changeType == 2 || changeType == 3" width='130' show-overflow-tooltip></el-table-column>
             ...
             <!-- <el-table-column label="销售单价(不含税)"  min-width='100' show-overflow-tooltip>
                <template scope="scope">
                  ¥{{scope.row.salePriceUnTax}}{{scope.row.salePriceUnit}}
                </template>
             </el-table-column> -->
             <el-table-column label="销售单价" min-width='120' show-overflow-tooltip>
                <template scope="scope">
                  ¥{{scope.row.salePrice}}{{scope.row.salePriceUnit}}
                </template>
             </el-table-column>
             <el-table-column label="采购商税点" min-width='120'>
                <template scope="scope">
                  {{scope.row.taxPoint}}%
                </template>
             </el-table-column>
             ...
             <el-table-column min-width='120' label="成本价(不含税)" v-if="orderDetail.type != '2'" show-overflow-tooltip>
                <template scope="scope">
                  ¥{{scope.row.priceUnTax}}{{scope.row.priceUnit}}
                </template>
             </el-table-column> -->
             <el-table-column class-name="red" min-width='140' :label="(changeType == 0 || changeType == 2) ? '退货匹号(至少选一匹)' : '换货匹号(至少选一匹)'" show-overflow-tooltip>
                <template scope="scope">
                  ...
                  <div><el-button size="mini" type='info' @click.native="choosePihao(scope.row, scope.$index)">选择</el-button></div>
                </template>
             </el-table-column>
           </el-table>
  ...
  <!-- 退货 -->
    <template>
     <div  v-if="changeType == 2 || changeType == 3">
      ...
      <div class="line-item clearfix">
        ...
        <div class="line-item-r">
           <span style="padding-right: 5px;">{{returnNumber}}</span>{{orderDetail.colorList && orderDetail.colorList[0] && orderDetail.colorList[0].priceUnit.split('/')[1]}}
        </div>
      </div>
      <div class="line-item clearfix">
        <div class="line-item-l">
        ...
        <div class="line-item-r" v-if="returnMoney">
      ...
      <div class="line-item clearfix">
        <div class="line-item-l">
          <span><b class="mark">*</b>{{orderDetail.a}}搜芽退款采购商</span>
        </div>
        <div class="line-item-r">
      ...
      <div class="line-item clearfix">
        <div class="line-item-l">
          <span>{{orderDetail.a}}其中扣服务费</span>
        </div>
        <div class="line-item-r">
           <y-input size="small"
           type="number"
    ...
    <div v-if="false">
      ...
      <div class="line-item clearfix">
        ...
        <div class="line-item-r">
          ...
          <span class="unit">{{orderDetail.colorList && orderDetail.colorList[0] && orderDetail.colorList[0].priceUnit.split('/')[1]}}</span>
        </div>
      </div>
      <div class="line-item clearfix">
        <div class="line-item-l">
        ...
        <div class="line-item-r">
           ...
           <span class="unit">{{orderDetail.colorList && orderDetail.colorList[0] && orderDetail.colorList[0].salePriceUnit}}</span>
        </div>
      </div>
   ...
   <!-- changeType == '0' || (changeType == '1' && buyerPay == '1' 原来判断 -->
    ...
    <div class="line-item clearfix">
      <div class="line-item-l">
        <span>跟单<template v-if="changeType == '0'">退</template><template v-if="changeType == '1'">换</template>货备注</span>
      </div>
      <div class="line-item-r">
...
export default {
  ...
  methods: {
    changeCurrent (selection, row) {
    },
    selectData(row, index) {
      let sameOutReposityNumber
      if (this.multipleSelection.length > 0) {
        this.multipleSelection.forEach((item) => {
           sameOutReposityNumber = item.outReposityNumber
        })
        if (row.pihao) {
          console.log('sameOutReposityNumber', sameOutReposityNumber)
          if (row.outReposityNumber == sameOutReposityNumber) {
            return true
          } else {
            return false
          }
    ...
    convertData (obj) {
      ...
      if (this.colorList.length == 0) {
        ...
        this.$message({
          type: 'error',
          message: '该订单中仍有出仓单未对账，请对账后再申请售后退换货',
          duration: 1500,
          onClose () {
    ...
    collectDatas () {
      ...
      req.followerRemark = this.followerRemark
      ...
      if (this.changeType == 2 || this.changeType == 3) { // 售后
        ...
        req.moneySoouyaToBuyer = this.soouyaPayToBuyer
      }
      return req
    },
    handleSubmit () {
      ...
      request({
        ...
        if (res.success == 1) {
          ...
          this.$message({
            ...
            duration: 1000,
            onClose () {
              that.$router.replace({ name: 'deliver' })
            }
          })
        } else {
          this.$message({
    ...
    confirmPihao () {
      ...
      this.colorList.map((val, index) => {
        let temCurrent = (val.productNumber + val.color + index)
    ...
    // 弹窗内选择匹号
    choosePihao (obj, index) {
      const that = this
      this.clothLoneIdList = []
      this.changepiShow = true
      this.tempMark = `${obj.productNumber}${obj.color}${index}`
  ...
  computed: {
   commonFn () {
    ...
    leftnumber () {
      return 500 - Number(this.followerRemark.length)
    },
    // 提交校验规则
    isSubmit () {
      this.count++
  ...
  watch: {
    ...
    followerRemark (val) {
      if (val.length >= 500) {
        this.followerRemark = val.slice(0, 500)
      }
    },
