      ...
      <el-form label-width="120px" label-position="right">
        ...
        <template v-if="radio1 == 10">
          ...
          <el-form-item label="退货明细" :required="true">
            <el-table :data="obj.selectList" border style="margin-top:12px; width:1110px" ref="detailTable">
        ...
        <el-form-item label="已出仓销售货款">
          <template v-if="obj.order.type == 1">
            <span>￥{{serviceSaleMoney | formatMoney}}</span>
          </template>
          <template v-if="obj.order.type == 2">
            <span>￥{{tradingSaleMoney | formatMoney}}</span>
          </template>
        </el-form-item>
        <template v-if="radio1 == 10">
...
export default {
  ...
  mounted() {
    this.serviceSaleMoney = 0
    this.tradingSaleMoney = 0
  ...
  methods: {
    ...
    getData() {
      newRequest({
        url: '/redwood/reposity/OutReposity/getOutReposityForReplaceReturn',
        method: 'get',
        ...
        this.$store.dispatch('changeload', false) // 设置停止加载
      })
    ...
    // 处理提交按钮
    confirmEditSubmit() {
      let len = this.obj.selectList.length - 1
      let hadInput = true
      for (let i = 0; i < len; i++) {
        let inputList = {
          color: '',
        ...
        if (!this.obj.selectList[i].inputQuantity && this.obj.selectList[i].selectQuantityCount != 0) {
          hadInput = false
          break
        } else if (this.obj.selectList[i].selectQuantityCount != 0) {
          inputList.color = this.obj.selectList[i].color
          inputList.quantity = this.obj.selectList[i].inputQuantity
          this.requestParams.returnRealQuantity.push(inputList)
        }
      }
      if (!hadInput) {
        this.$message('请填写退货/换货实数')
      } else {
        if (this.radio1 == 11) {
      ...
      rows.clothLoneList.forEach(row => {
        ...
        if (row.isChecked) {
          ...
          if (!exist) {
            list.push(row.id)
            if (row.quantity) {
              this.serviceSaleMoney += row.quantity * rows.salePrice
            } else {
              this.serviceSaleMoney += 0 * rows.salePrice
            }
            if (row.realQuantity) {
              this.tradingSaleMoney += row.realQuantity * rows.salePrice
            } else {
              this.tradingSaleMoney += row.realQuantity * rows.salePrice
            }
          }
    ...
    calInputQuantity() {
      let len = this.obj.selectList.length
      this.obj.selectList[len - 1].inputQuantity = 0
      for (let i = 0; i < len - 1; i++) {
        if (!this.obj.selectList[i].inputQuantity) {
        } else {
          this.obj.selectList[len - 1].inputQuantity += parseFloat(this.obj.selectList[i].inputQuantity)
        }
        console.log(i)
      }
      // console.log(this.obj.selectList)
