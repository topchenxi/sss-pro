            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item,index) in list" :key="index">
                  ...
                  <!-- 审核时间 -->
                  <td>
                    <p>{{item.scoreTime | date('yyyy-MM-dd')}}</p>
                    <p>{{item.scoreTime | date('hh:mm:ss')}}</p>
                  </td>
                  <!-- 采购商 -->
                  <td class="ta-l">
                  ...
                  <td>
                    <div v-if="!item.isEdit">
                      {{item.changeCard | changeCardFilter}}
                    </div>
                    <div v-else>
                      <el-select v-model="item.changeCard">
                        <el-option v-for="item in [{value: 1, name: '是'}, {value: 0, name: '否'}]" :label="item.name" :value="item.value"></el-option>
                      </el-select>
                    </div>
                  </td>
                  <!-- 样板 -->
                  ...
                  <td class="ta-c">
                    <el-button class="table-btn submit" :disabled="!item.changeCard" @click.native="toChangeCard(item)">换卡头</el-button>
                    <el-button class="table-btn" v-if="!item.isEdit" @click.native="edit(item)">编辑</el-button>
                    <el-button class="table-btn" v-else @click.native="save(item)">保存</el-button>
                  </td>
                </tr>
              </tbody>
            </table>
...
import { getCardList, updateChangeCard } from 'src/service/dataer';
export default {
  components: {
    hsMenu,
    search
  ...
  methods: {
    ...
    async save(item) {
      const params = {
        ...
        'changeCard': item.changeCard
      }
      try {
        let dlgRes = await this.$confirm('是否保存数据？', '提示');
        if (dlgRes !== 'confirm') return;
        let res = await updateChangeCard(params);
    ...
    toChangeCard(item) {
      this.$router.push({ name: 'changeCardEdit', query: { id: item.orderNumber } })
    },
    async getList() {
