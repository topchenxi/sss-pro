  ...
  <section min-width="1300">
    ...
    <div class="form-box">
      <el-form label-position="right" label-width="120px">
              ...
              <template v-for="(item1,index1) in obj.beforeList">
                <tr>
                  ...
                  <td style="text-align:center;" :rowspan="item1.clothLoneList.length + 2">
                    <el-input v-model="item1.inputQuantity" style="width:100px" @change="calMoney"></el-input>
                  </td>
                  <template v-if="obj.type == 2 ||obj.type == 4">
                    <td style="text-align:center;" :rowspan="item1.clothLoneList.length + 2">
                      <el-input v-model="item1.inputRealQuantity" style="width:100px" @change="calMoney"></el-input>
                    </td>
                  </template>
        ...
        <template v-if="obj.type==1 || obj.type ==3">
          <el-form-item label="供应商退款" :required="true">
            <el-input style="width:200px;margin-top:12px" v-model="requestParams.sellerRefund"></el-input>
            <span>元</span>
          </el-form-item>
        </template>
        <template v-else>
          <template v-if="nowQuantityMoney > realQuantityMoney">
            <el-form-item label="搜芽补款" :required="true">
        ...
        <template v-if="obj.type == 2 || obj.type == 4">
          <el-form-item label="换货码单" style="margin-top:15px;" :required="true">
...
export default {
  ...
  methods: {
    ...
    confirmSubmit() {
      if (this.requestParams.sellerRefund && this.requestParams.sellerRefund != -1) {
        if (this.requestParams.sellerRefund > 999999) {
          this.$message('供应商退款不得超过999999')
        } else if (this.requestParams.sellerRefund < 0 && this.requestParams.sellerRefund != -1) {
          this.$message('供应商退款不得少于0')
        }
        this.requestParams.moneySoouyaToSeller = -1
      }
      if (this.requestParams.moneySoouyaToSeller && this.requestParams.moneySoouyaToSeller != -1) {
        if (this.requestParams.moneySoouyaToSeller > 999999) {
          this.$message('搜芽补款不得超过999999')
        } else if (this.requestParams.moneySoouyaToSeller < 0 && this.requestParams.moneySoouyaToSeller != -1) {
          this.$message('搜芽补款不得少于0')
        }
    ...
    calMoney() {
      this.obj.needBuyQuantity = 0
      this.obj.realQuantity = 0
      this.nowQuantityMoney = 0
      this.realQuantityMoney = 0
      for (let i = 0; i < this.obj.beforeList.length; i++) {
        let num1 = 0
        let num2 = 0
        if (!this.obj.beforeList[i].inputQuantity) {
          ...
          if (this.obj.afterList.length) {
            this.obj.afterList[i].realQuantity = 0
          }
        } else {
          num1 = this.obj.beforeList[i].inputQuantity
          if (this.obj.afterList.length) {
            this.obj.afterList[i].realQuantity = this.obj.beforeList[i].inputQuantity
          }
        }
        this.obj.realQuantity += num1
        if (!this.obj.beforeList[i].inputRealQuantity) {
          num2 = 0
          if (this.obj.afterList.length) {
            this.obj.afterList[i].needBuyQuantity = 0
          }
        } else {
          num2 = this.obj.beforeList[i].inputRealQuantity
          if (this.obj.afterList.length) {
            this.obj.afterList[i].needBuyQuantity = this.obj.beforeList[i].inputRealQuantity
          }
        }
        for (let j = 0; j < this.obj.order.colorList.length; j++) {
          if (this.obj.order.colorList[j].color == this.obj.beforeList[i].color) {
            this.realQuantityMoney += num1 * this.obj.order.colorList[j].price
            this.nowQuantityMoney += num2 * this.obj.order.colorList[j].price
            this.obj.needBuyQuantity += num2
          }
        }
      }
      if (this.realQuantityMoney >= this.nowQuantityMoney) {
        this.requestParams.sellerRefund = (this.realQuantityMoney - this.nowQuantityMoney).toFixed(2)
        this.requestParams.moneySoouyaToSeller = -1
      } else {
        this.requestParams.moneySoouyaToSeller = (this.nowQuantityMoney - this.realQuantityMoney).toFixed(2)
        this.requestParams.sellerRefund = -1
      }
    },
    calWordCount() {
