    ...
    <div class="form-box">
      <el-form label-position="right" label-width="120px">
        ...
        <template v-if="obj.moneyType == 1">
          <el-form-item label="供应商退款">
            <span>{{obj.money | formatMoney}}</span>
          </el-form-item>
        </template>
        <template v-if="obj.moneyType == 2">
          <el-form-item label="搜芽补款">
            <span>{{obj.money | formatMoney}}元</span>
          </el-form-item>
        </template>
        <template v-if="obj.type == 4 || obj.type == 2">
        ...
        <el-form-item label="原销售货款值" :required="true">
          <input style="width:200px; height:25px; margin-top:12px; border-radius:3px; border:1px solid #ccc;" type="number" @input="checkMinMaxVal()" v-model="editMoney"></input>
          <span>元</span>
        </el-form-item>
        <template v-if="obj.type == 3">
          <el-form-item label="跟单员退货备注">
            <el-input type="textarea" v-model="editData.followerRemark" style="width:250px; margin-top:12px"></el-input>
          </el-form-item>
        </template>
        <template v-else>
        ...
        <el-form-item style="margin-top:50px">
          <el-button type="primary" size="small" @click.native="confirmEdit">确定</el-button>
          <el-button size="small" @click.native="toBack" style="margin-left:50px">取消</el-button>
        </el-form-item>
      </el-form>
    </div>
    <lightbox></lightbox>
...
export default {
  ...
  methods: {
    getData() {
      ...
      newRequest({
        ...
        if (res.success == 1) {
          ...
          this.editData.followerRemark = this.obj.followerRemark
          this.editMoney = this.obj.soouyaPayToBuyer
          ...
          // this.rrMadanList.push(this.imgPath + '/upload/redwood/madan/557304d5-9648-45d5-9890-d83d24d2dd10.jpg')
        } else {
    ...
    confirmEdit() {
      ...
      this.editData.soouyaPayToBuyer = this.editMoney
      this.editData.id = this.requestParams.id
      newRequest({
        url: '/redwood/returnreplace/editSoouyaToBuyerMoney',
        data: this.editData,
        method: 'post',
    ...
    checkMinMaxVal() {
      this.editMoney = parseFloat(this.editMoney)
      if (this.editMoney < 1) {
        this.editMoney = 1
      }
      if (this.editMoney > 999999) {
        this.editMoney = 999999
      }
      this.editData.soouyaPayToBuyer = this.editMoney
    },
    toBack() {
