            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item,index) in list" :key="index">
                  ...
                  <!-- 采购商 -->
                  <td class="ta-l">
                    <span class="sign" v-if="item.customerCompany&&item.customerCompany.indexOf('代采') != -1">代采</span> {{item.customerCompany}}
                  </td>
                  <!-- 品类 -->
                  ...
                  <td>
                    <el-button @click.native="toDetail(item)" class="table-btn">详情</el-button>
                    <el-button v-if="item.status == 103" @click.native="cancel(item)" class="table-btn">取消</el-button>
                  </td>
                </tr>
              </tbody>
            </table>
...
import { getFindingList, chgFindOrderCancel } from 'src/service/assessor';
export default {
  components: {
    hsMenu,
    search
  ...
  methods: {
    toDetail(item) {
      this.$router.push({ name: 'checkFindingDetail', query: { id: item.orderNumber } })
    },
    async cancel(item) {
      try {
        let dlgRes = await this.$confirm('是否取消订单?', '提示');
        if (dlgRes !== 'confirm') return;
        let res = await chgFindOrderCancel({ 'orderNumber': item.orderNumber });
    ...
    async getList() {
      ...
      let res = await getFindingList(this.params);
      if (res.success !== '1') return;
