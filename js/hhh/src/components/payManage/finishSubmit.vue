      ...
      <div class="sy-order-wrap" v-for="(order,orderIdx) in orderList" v-show="order.isShow">
        <template v-if="order.category === 'all-all'">
          ...
          <el-table :data="order.dhOrder.outReposityList">
            ...
            <el-table-column label="出仓销售货款" align="center">
              <template scope="scope">
                {{scope.row.saleMoney | formatMoney}}
              </template>
            </el-table-column>
            <el-table-column label="应收出仓销售货款" align="center">
              <template scope="scope">
            ...
            <el-table-column label="提交结算时间" prop="submitTime" :formatter="formatTime" align="center"></el-table-column>
          </el-table>
        </template>
