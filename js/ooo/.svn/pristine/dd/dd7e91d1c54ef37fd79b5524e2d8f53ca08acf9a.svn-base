  ...
  <section v-loading.body="fullScreenLoading">
    ...
    <div class="el-bulkTable-content">
      <el-table :data="tableData" :height="tableHeight" border>
        <el-table-column label="入仓单号" prop="number" width="120" align="left"></el-table-column>
        <el-table-column label="入仓时间" prop="createTime" width="160" align="left" :formatter="formatTime"></el-table-column>
        <el-table-column label="大货类型" prop="" width="100">
          <template slot-scope="scope">
            <span v-if="scope.row.bulk.type == 1">服务单</span>
            <span v-if="scope.row.bulk.type == 2">贸易单</span>
            <span v-if="scope.row.bulk.type == 3">采购单</span>
          </template>
        </el-table-column>
        <el-table-column label="订单号" prop="bulk.number" width="200" align="left"></el-table-column>
        <el-table-column label="验货地点" prop="reposityName" width="160" align="left"></el-table-column>
        ...
        <el-table-column label="品类" prop="" width="80">
          <template slot-scope="scope">
            <span v-if="scope.row.bulk.category == 1">面料</span>
            <span v-if="scope.row.bulk.category != 1">辅料</span>
          </template>
        </el-table-column>
        <el-table-column label="采购数量" width="140" align="right">
          <template slot-scope="scope">
            <span>{{scope.row.bulk.quantity | formatNumber}}{{scope.row.bulk.quantityUnit}}</span>
          </template>
        </el-table-column>
        <el-table-column label="待验货数量" width="160" prop="num" align="right"></el-table-column>
