      ...
      <template v-for="(item, index) in tableData">
        <div @click="getClothLone(index)" class="fold-content" :key="index">
          ...
          <el-row :gutter="10">
            ...
            <el-col :span="7">
              <span>订单发布时间：{{item.createTime | formatTime}}</span>
            </el-col>
            <el-col :span="8">
              <span>订单总金额：
          ...
          <div v-show="item.showDetail" style="" class="fold-content-wrap">
            <template v-if="item.clothLoneList && item.clothLoneList.length">
              <template v-for="(clothItem,clothIndex) in item.clothLoneList">
                <div :key="clothIndex" style="padding: 5px 15px;">
                  <span class="icon-ff"></span>
                  <span>出仓单号：{{clothItem.number}}</span>
                  <span class="left-margin25">待发货数量：{{clothItem.num}}</span>
                </div>
              </template>
            </template>
            <template v-else>
