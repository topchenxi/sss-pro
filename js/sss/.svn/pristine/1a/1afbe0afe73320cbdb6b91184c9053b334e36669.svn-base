          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item,index) in list" :key="index">
                ...
                <!-- 订单状态 -->
                <td> <span class="green" v-if="item.status === 20">待审核</span> <span class="error" v-else-if="item.status === 25">审核不通过</span> </td>
                <!-- 跟单员 -->
                <td>{{item.followerName}}</td>
                <!-- 操作 -->
                <td> <span @click="$router.push('/audit/find/details?id=' + item.id)" v-if="item.followerName" class="icon icon-editor m10"></span> </td>
              </tr>
            </tbody>
          </table>
        </div>
...
import { getOrderListByFind } from 'src/common/service';
export default {
  components: {
    AuditTabs
  },
  ...
  methods: {
    async getList() {
      this.loading = true;
      let res = await getOrderListByFind(this.params);
      this.loading = false;
      if (res.success == '1') {
