          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item,index) in tableData" :key="index">
                ...
                <!-- 操作 -->
                <td>
                  <span @click="$router.push('/audit/cut/details?id=' + item.id)" v-if="item.followerName" class="icon icon-editor m10"></span>
                </td>
              </tr>
            </tbody>
          </table>
...
import { getOrderListByCut } from 'src/common/service';
export default {
  components: {
    AuditTabs
  },
  ...
  methods: {
    async getList() {
      this.loading = true;
      let res = await getOrderListByCut(this.params);
      this.loading = false;
