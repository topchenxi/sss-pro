...
<div v-loading.fullscreen="fullscreenLoading" class='payTransaction-list'>
  ...
  <p class="control" style="margin-bottom: 10px;">
    <span class="subtitle is-6">支付笔数：<span style="color:red">{{payTransactionList.count||0.00}}</span> 笔，支付金额：<span style="color:red">{{Number(payTransactionList.total).toFixed(2)||0.00}}</span>元</span>
  </p>
    ...
    <el-table :data="payTransactionList.result" border  :height="height" >
      ...
      <el-table-column inline-template label="商户订单号" min-width="180">
        <div>
          ...
          <div v-else>
              <span v-for="item in row.serviceNumberList">{{item}}</span>
          </div>
        </div>
      </el-table-column>
      <el-table-column property="transactionNo" label="支付流水号" min-width="160" show-tooltip-when-overflow></el-table-column>
      <el-table-column property="yfbTransactionNo" label="易付宝交易号" min-width="130" show-tooltip-when-overflow></el-table-column>
      <el-table-column property="accountName" label="收款方姓名" min-width="110" show-tooltip-when-overflow></el-table-column>
      <el-table-column property="accountType" label="收款方账户类型" min-width="130" :formatter="getAccountTypeStr"  show-tooltip-when-overflow></el-table-column>
      <el-table-column property="accountBankShort" label="收款方银行编号" min-width="130" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="accountBranch" label="收款方银行名称" min-width="130" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="accountBankId" label="联行号" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="accountProvince" label="收款方开户行省" min-width="130" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="accountCity" label="收款方开户行市" min-width="130" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="accountNumber" label="收款方银行账号" min-width="130" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="toPayMoney" label="金额（元）" min-width="100" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="fee" label="手续费（元）" min-width="100" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="orderName" label="订单名称" min-width="100" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="userNumber" label="供应商ID" min-width="100" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="userName" label="供应商名称" min-width="150" show-tooltip-when-overflow> </el-table-column>
      <el-table-column property="payStatus" label="支付状态" min-width="150" :formatter="payStatusStr" show-tooltip-when-overflow> </el-table-column>
    </el-table>
    <div class="page">
