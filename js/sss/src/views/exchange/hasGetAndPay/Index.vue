  ...
  <el-tabs type="border-card"  @tab-click="switchTab" style = "width: 100%;">
    ...
    <el-tab-pane label="已退款">
      ...
      <el-table class="fixed-table" :data="result" border :height="height">
        <el-table-column inline-template show-tooltip-when-overflow
        ...
        <el-table-column property="returnReplaceNumber" label="退换货单号" min-width="170" show-tooltip-when-overflow></el-table-column>
        <el-table-column property="serviceNumber" label="订单号" min-width="170" show-tooltip-when-overflow></el-table-column>
        <el-table-column v-if='currentTab == 1' inline-template label="供应商退款/元"  min-width="140" show-tooltip-when-overflow>
           <span>{{row.refundMoney > 0 ? "+" : ''}}{{Number(row.refundMoney).toFixed(2)}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab == 1' inline-template label="到账金额/元"  min-width="140" show-tooltip-when-overflow>
          <span>{{row.incomeMoney > 0 ? "+" : ''}} {{Number(row.incomeMoney).toFixed(2)}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab == 2' inline-template label="销账金额/元"  min-width="140" show-tooltip-when-overflow>
           <span>{{row.xiaozhangMoney > 0 ? "+" : ''}}{{Number(row.xiaozhangMoney).toFixed(2)}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab == 2' inline-template label="实收退款金额/元"  min-width="140" show-tooltip-when-overflow>
           <span>{{row.shishouTkMoney > 0 ? "+" : ''}}{{row.shishouTkMoney === null ? '--' : Number(row.shishouTkMoney).toFixed(2)}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab == 2' inline-template label="实付销账金额/元"  min-width="140" show-tooltip-when-overflow>
           <span>{{row.shifuXzMoney > 0 ? "+" : ''}}{{row.shifuXzMoney === null ? '--' : Number(row.shifuXzMoney).toFixed(2)}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab == 3' inline-template label="退款金额/元"  min-width="140" show-tooltip-when-overflow>
          <span>{{row.refundMoney > 0 ? "+" : ''}}{{Number(row.refundMoney).toFixed(2)}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab == 2' property="batchNumber" label="批次号" min-width="120" show-tooltip-when-overflow> </el-table-column>
        <el-table-column property="sellerCompany" label="供应商" min-width="120" show-tooltip-when-overflow> </el-table-column>
        ...
        <el-table-column inline-template label="买货员" min-width="120" show-tooltip-when-overflow>
          <div>{{row.purchaserName}}/{{row.purchaserTel}}</div>
        </el-table-column>
        <el-table-column inline-template label="退换类型" min-width="100" show-tooltip-when-overflow >
          <span>{{replaceOrReturnTypeList[row.returnReplaceType - 1]}}</span>
        </el-table-column>
        <el-table-column v-if='currentTab != 3' inline-template property="returnReplaceCreateTime" label="申请退换货时间" min-width="170" show-tooltip-when-overflow>
          <span>{{formatTime(row.returnReplaceCreateTime)}}</span>
        </el-table-column>
        ...
        <el-table-column inline-template label="操作" width='100' fixed="right">
          <el-button size='mini' @click.native='jumpToDebtDetail(row.id)'>明细</el-button>
        </el-table-column>
      </el-table>
  </el-tabs>
  <div class="page">
