...
//是否从缓存中取模板逻辑处理
var appVersion
var getTmplFromCache
(function () {
	try {
		$(document).find('script').each(function (i, n) {
		    var $n = $(n)
		    var scriptSrc = $n.attr('src')
		    //scriptSrc = 'javascripts/shared-dbd9c9a3b02dbf97c9a3.js'
		    if (i === 0) {
		    	var scriptSrcArray1 = scriptSrc.split('.js')
		    	if (scriptSrcArray1[0]) {
		    		var scriptSrcArray2 = scriptSrcArray1[0].split('-')
		    		if (scriptSrcArray2[1]) {
		    			appVersion = scriptSrcArray2[1]
		    			var oldAppVersion = localStorage.getItem('appVersion')
		    			if (appVersion === oldAppVersion) {
		    				getTmplFromCache = true
		    				//getTmplFromCache = false
		    			} else {// 清除旧缓存并更新缓存
	    ...
	    render: function(name, ctx, cb) {
	        ...
	        try {
	        	return new Promise((resolve, reject) => {
		        	var tmplUrl = '/templates/' + name

		        	var getAndRenderTmpl = function () {
		        		const requestStartTime = new Date().getTime()
			        	$.get(tmplUrl + '?_time=' + requestStartTime, function (res) {
				        	syncResult = _this.renderString(res, ctx)
				        	if (syncResult) {
				        		setCache({
				        			type: 'localStorage',
				        			key: tmplUrl,
