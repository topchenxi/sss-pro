<template id="">
  <div class="content">
    ...
    <div class="content">
      <h4>
        订单详情
        <a class="print-btn" :href="'/common/printDetail/'+(detailData[0] && detailData[0].order.orderNumber)" target="_blank">
        ...
        <el-table :data="detailData" border v-if="detailData.length">
          ...
          <el-table-column inline-template label="大货类型" width="100">
            <template scope="scope">
              <template v-if="scope.row.type == 1">服务单</template>
              <template v-if="scope.row.type == 2">贸易单</template>
              <template v-if="scope.row.type == 3">采购单</template>
            </template>
          </el-table-column>
          ...
          <el-table-column property="order.invoiceStatus" label="采购发票" :formatter="getInvoiceStatusStr" v-if="detailData.length && (detailData[0].order.category == 'all-all' || detailData[0].order.category == 'dh-zy')" width="100"></el-table-column>
          <el-table-column inline-template label="税点" width="100" show-tooltip-when-overflow>
            <div>{{row.order.taxPoint}}</div>
          </el-table-column>
          <el-table-column property="order.payInvoiceInneed" label="供应商发票" width="120" :formatter="getPayInvoiceInneedStr" v-if="detailData.length && (detailData[0].order.category == 'all-all' || detailData[0].order.category == 'dh-zy')"></el-table-column>
        ...
        <div class="content" style='margin-top: 20px;'>
          ...
          <div class="box">
            <article class="media" v-if="detailData.length">
              <a :href="val.url" class="image media-left is-64x64" v-lightbox="madan" v-for="val in madan">
