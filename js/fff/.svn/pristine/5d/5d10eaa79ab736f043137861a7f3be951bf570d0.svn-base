    ...
    <div class="box">
      ...
      <div class="upload">
        ...
        <el-row v-for="(file, index) in fileList" class="file-item">
          <el-col :span="3" :key="file + index">
            <el-card :style="{float:'left'}" v-loading="file.status != 'finished'">
              <img :src="('http://test.soouya.com' + file.response.imgUrl)" class="image"  v-if="file.status == 'finished'">
              <div class="mark">
                <p>{{file.name}}</p>
