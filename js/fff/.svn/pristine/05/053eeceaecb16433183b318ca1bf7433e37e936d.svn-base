...
export default {
  ...
  mounted() {
    ...
    // this.customerId = '5c883311-d2c6-4d9b-8881-eea1a2216411' // 临时代码
    if (this.customerId) {
      this.getData()
    } else {
  ...
  methods: {
    getData() {
      ...
      this.loanTime = Number(this.$route.query.loanTime)
      if (this.loanTime) {
        let lT = new Date(this.loanTime).getTime()
        let lY = new Date(lT).getFullYear()
        let lM = new Date(lT).getMonth() + 1
        let lD = new Date(lT).getDate()
        let eT = new Date(this.tableData.expectedRepaymentTime).getTime()
        let eY = new Date(eT).getFullYear()
        let eM = new Date(eT).getMonth() + 1
        let eD = new Date(eT).getDate()
        let lTime = lY + '-' + lM + '-' + lD
        let eTime = eY + '-' + eM + '-' + eD
        let subT = (new Date(eTime).getTime() - new Date(lTime).getTime()) / 3600 / 24 / 1000
        console.log(subT)
        subT = parseInt(subT)
        this.tableData.factoringDate = subT
        console.log(subT)
      }
      newRequest({
